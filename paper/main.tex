...
that reduction). \pend{We expect \method{} to sit at markedly lower $\faph$ for
comparable $\bar F_1$, with the gap widening as the recall target rises and as
front-end degradation increases.}

\subsection{Calibration}
\input{tables/calibration}
\Cref{tab:calib} tests H3 (M3 sharply reduces $\ece$ with no recall cost).
\pend{We expect raw fused belief to be over-confident in the mid range and
isotonic calibration to cut $\ece$ several-fold, with adaptive-$\ece$ showing
the same trend on the score mass that matters.}

\subsection{Risk control}
\input{tables/risk_control}
\Cref{tab:rc} tests H4 (M4 holds the budget on held-out normal streams). For
each $\beta$ we report the chosen $\tau$, whether the budget was certified on
the calibration hours, and the empirical $\faph$ on a disjoint normal test
stream. \pend{We expect certification wherever normal hours exceed
$\approx\tfrac{1}{2}\chi^2_{0.95}(2)/\beta$ and the held-out rate to fall at or
below $\beta$ in every certified case.}
...
\subsection{Cost}
\input{tables/cost}
\Cref{tab:cost} reports the decision-layer latency added on CPU and the
end-to-end frame rate with the pose front end. \pend{We expect the M1--M4
layer to add on the order of a millisecond per frame and the full system to
exceed 15\,fps on a commodity CPU.}
