...
pending}. Its official release ships only raw video (76.6\,GB); we also
checked the original authors' own released tracking bundle (a single Drive
file linked from their code repository) as a possible shortcut around a GPU
pass, and confirmed by inspection that it contains YOLO-style bounding-box
tracks (class IDs 0/1/2/3/5/7 -- COCO person/vehicle) with \emph{no keypoints}
-- sufficient for none of M1 (needs per-joint confidence) or the FALL/FIGHT
cues (need limb geometry). We also checked whether at least a small,
honestly-labelled subset of test clips could be pose-extracted without the
full download: the official video release turned out to be split across a
106-volume \texttt{.7z} multi-part archive with no per-clip boundaries, so
even one clip requires fetching and reassembling the entire 76.6\,GB
archive first -- there is no partial-download path. Both routes checked and
ruled out, a real pose-based result on this benchmark needs a GPU
pose-extraction pass over the full video release, which remains outside a
CPU-only session's scope.
...
(\S\ref{sec:m4} results). Two further sweeps are run on all four benchmarks
...
\label{sec:ablations}
\input{tables/ablation_fusion}
\input{tables/ablation_calibration}
...
inconsequential on all four benchmarks}: AUC and $\bar F_1$ match to within
$0.001$--$0.002$ everywhere, because in practice at most one cue is
strongly active on a given frame, and the two rules only disagree when
several cues fire at comparable strength simultaneously. \textbf{Removing
fusion altogether (B4) is a different matter, and its cost is
dataset-dependent}: on ShanghaiTech and UBnormal, B4 tracks B0 almost
exactly (one cue dominates the belief on those streams already, so summing
in the rest adds little); on CHAD, B4 loses $1.2$ AUC points against the
full fusion ($0.583$ vs.\ $0.590$), the one case in our four benchmarks
where combining cues measurably helps. We read this as evidence that
...
calibration maps. \textbf{Isotonic regression wins on every one of the four
benchmarks}, and the margin is not small: it more than halves Platt's $\ece$
on CHAD and Avenue, and is one order of magnitude better than Platt on
ShanghaiTech and UBnormal ($0.03$ vs.\ $0.56$; $0.05$ vs.\ $0.47$).
Temperature scaling -- a single learned scalar -- lands consistently between
the two, closer to Platt than to isotonic. The pattern matches what produces
it: Platt and temperature scaling both assume the belief-to-probability map
is a sigmoid (Platt) or a fixed monotone rescaling (temperature) of the raw
score, while isotonic makes no shape assumption, and the fused Dempster
belief on these streams is visibly non-sigmoidal (heavily mass-concentrated
near $0$ with a long tail, from \S\ref{sec:m2}'s discounting). \textbf{This
is the clearest actionable finding among the ablations}: a deployer of
\method{} should keep isotonic as the default calibration map rather than
substitute a parametric one for simplicity, since the cost of doing so is
large and consistent, not occasional.
...
$170.06/(170.06+1.384)\approx99.2\%$ of the per-frame budget, so end-to-end
...
\method{}'s $1.38$\,ms/frame cost unchanged.

\subsection{Qualitative}
\input{figures/qual_examples}
