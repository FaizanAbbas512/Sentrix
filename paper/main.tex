...
\paragraph{Datasets}
All five are complete: ShanghaiTech \cite{shanghaitech2017} and UBnormal
\cite{ubnormal2022} (official test splits, 213 and 211 clips), CHAD
\cite{chad2023} (official \texttt{test\_split\_1}, 134 clips), Avenue
\cite{avenue2013} (21 test clips), and NWPU-Campus \cite{nwpu2023} (official
test split, 242 clips). NWPU-Campus was the hardest to obtain: its release
ships only raw video (76.6\,GB, split across a 106-volume \texttt{.7z}
...
$\ece$ falls by $93\%$ (ShanghaiTech), $91\%$ (UBnormal), $77\%$ (CHAD),
...
the five} with our current cue set. \textbf{Decision-layer cost is stable
and cheap wherever it runs}: $1.22$--$1.47$\,ms/frame across the four
...
is dataset-dependent}: on ShanghaiTech, UBnormal and NWPU-Campus, B4 tracks
B0 almost exactly (one cue dominates what little belief there is, so summing
in the rest adds little); on CHAD, B4 loses $1.2$ AUC points against the
full fusion ($0.583$ vs.\ $0.590$), the one case in our five benchmarks
...
Platt on ShanghaiTech and UBnormal ($0.03$ vs.\ $0.56$; $0.05$ vs.\ $0.47$).
...
independent benchmarks -- the first time this has been demonstrated at all
for a heuristic multi-cue video anomaly detector -- all at a CPU cost of about
...
not as a failure to hide. Code and the evaluation harness are released to
make the protocol reusable; the one piece of this evaluation not yet
complete is extending the 12-pair cross-dataset generalisation matrix
(\Cref{tab:cross}) to include NWPU-Campus as a fit/test source.
