...
\textbf{B3}, a published pose-density model, is run wherever a fair
comparison is possible without reproducing a full training pipeline:
STG-NF's own released scores \cite{stgnf2023} for its two officially
supported benchmarks (ShanghaiTech, UBnormal) are scored through the
identical M3 calibration fit and deployable-evaluation protocol via
\texttt{calm/external\_scores.py} (\S\ref{sec:b3}). STG-NF does not support
CHAD, Avenue or NWPU-Campus, and we neither train nor fine-tune it ourselves
on any benchmark -- doing so would be a different scope of work from
evaluating our own decision layer -- so no B3 entry appears for those three.
...
\subsection{Comparison to a published external baseline (B3)}
\label{sec:b3}
\S\ref{sec:setup} explains why B3 is not run as a general head-to-head
detector comparison: a fair run needs cloning and training a published
method's full codebase, a different scope of work from evaluating our own
back end. We did, however, obtain STG-NF's own released scores
\cite{stgnf2023} for its two officially supported benchmarks (ShanghaiTech,
UBnormal; it does not support CHAD, Avenue or NWPU-Campus) and scored them
with \texttt{calm/external\_scores.py}, which applies the identical M3
calibration fit and reports frame AUC, event $\bar F_1$, $\faph$ and $\ece$
under the same calib/test protocol as \texttt{calm.harness.run()} -- M1's
reliability discount and M4's risk control do not apply, since both are
defined over fused, reliability-aware multi-cue belief that a single opaque
density score does not expose. \Cref{tab:b3} reports the result; $26$ of
ShanghaiTech's $107$ and $18$ of UBnormal's $211$ official test clips were
excluded for a frame-count mismatch between our released ground truth and
STG-NF's score length (listed by name in the released report; not
force-aligned).

\input{tables/b3_external}

Two findings, both expected, and both worth stating plainly rather than
downplaying. \textbf{STG-NF discriminates far better than CALM-VAD on the
metric it was trained for}: frame AUC of $0.840$ vs.\ $0.552$ on
ShanghaiTech and $0.721$ vs.\ $0.538$ on UBnormal -- unsurprising, since
STG-NF is a normalizing flow trained end-to-end on each benchmark's own
normal-stream density, while CALM-VAD's five cues are untrained,
hand-specified rules evaluated identically on every benchmark. This paper's
contribution is not proposed as a frame-AUC improvement over such methods
(\S\ref{sec:limits}), and this comparison confirms rather than contradicts
that framing. \textbf{The M3 calibration layer generalizes to an external,
opaque score, though less effectively than to CALM-VAD's own fused belief}:
STG-NF's raw score is, coincidentally, almost as miscalibrated as our own
raw belief on both benchmarks ($\ece=0.442$ vs.\ our $0.414$ on
ShanghaiTech; $0.506$ vs.\ our $0.499$ on UBnormal), and the identical
isotonic map lowers it substantially on both ($0.442\!\to\!0.153$;
$0.506\!\to\!0.072$), demonstrating that the calibration and
deployable-evaluation contributions are not specific to our own fusion
output. It does not close as far as it does for CALM-VAD's own belief
($0.020$ and $0.046$ after calibration), which we attribute to STG-NF's
score being an unbounded log-likelihood rather than a quantity already
loosely scaled toward $[0,1]$ by construction -- isotonic regression can
still monotonically remap it, but with less exploitable structure from the
same calibration-split size. Alarm load at the one recall level both
methods reach on UBnormal is, coincidentally, almost identical ($241.0$/h
STG-NF vs.\ $240.7$/h CALM-VAD at $\rho{=}0.7$) despite the large AUC gap,
suggesting that on this benchmark the achievable FAPH ceiling is set more by
UBnormal's own anomaly density and confirmed-normal scarcity
(\S\ref{sec:m4} results) than by which detector produces the ranking.
...
evaluated any.
\emph{Fixed cue thresholds do not appear to be the bottleneck.} The five
cues' thresholds (e.g.\ \texttt{loitering.dwell\_seconds},
\texttt{crowd.threshold}, \texttt{fight.proximity}) are fixed constants,
tuned once rather than adapted per site; we built and tested a
scene-adaptive variant that learns them from a site's own confirmed-normal
footage statistics only (\texttt{calm/scene\_autotune.py}, no anomaly
labels needed, so it is deployable the same way M4 self-calibrates). Three
of five benchmark releases have no fully-normal clip to learn from at all;
on the two that do, the learned thresholds moved substantially but changed
frame AUC by $+0.0005$ (UBnormal) and $-0.0053$ (NWPU-Campus), with event
$F_1$ unchanged in both. We report this honestly rather than claim a win:
whatever limits detection on these benchmarks, it is not simply that our
threshold constants are mistuned.
