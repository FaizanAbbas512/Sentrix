...
Lightweight, real-time surveillance intelligence for resource-constrained
deployments (e.g. Pakistani Safe-City). Ek single lightweight model (YOLO26)
+ tracking + **paanch behavioral analyzers** + ek **intelligent fusion layer**
jo sab signals ko milake threat level decide karta hai + live web dashboard.

## Detect kya karta hai (ab POSE-based — body dekhta hai, box nahi)
- **Posture** — har banda khada / gira hua (skeleton ke torso angle se)
- **Fall** — gira hua posture der tak = confirmed fall (box-ratio se bohot behtar)
- **Limb agitation** — haath/paaon ki ASLI speed (per-person, body-size normalized)
- **Fight** — do log paas + DONO ke limbs sach mein tez + der tak (akela tez = NAHI)
- **Loitering** — koi ek jagah zyada der (time + movement)
- **Crowd** — bheed (count threshold)

Pose model har person ka **17-point skeleton** deta hai — kandhe, kohni, kalai,
kulhe, ghutne, takhne. Isse system **actual body behaviour** dekhta hai, sirf
ek rectangle nahi. Video pe skeleton bhi draw hota hai.

> **Imaandar boundary:** Yeh system **ACTION** padhta hai (kya kar raha hai) —
> yeh real hai. **Niyat / dar / "mashkook nazar"** NAHI padhta — woh video se
> reliably possible hi nahi (science isay reject karti hai). Action real hai,
> mind-reading nahi.

> **Accuracy:** 100% nahi — koi system nahi hota. Realistic 85–95%. Paper mein
> limitations imaandaari se likhna hi credible banata hai.

### Pose model note
- Default `yolo26n-pose.pt`. Agar download na ho to `config.yaml` mein
  `yolo11n-pose.pt` kar do (woh pakka available hai, lagbhag waisa hi).
- Pose model sirf **person** deta hai — is mode mein **bags detect nahi** honge
  (abandoned object ke liye alag detection model chahiye, jo do model = slow).
  Behaviour pe focus karne ke liye pose hi best hai.

> Yeh project ek research paper ke liye banaya gaya hai. Code modular hai,
> `evaluate.py` se paper ke numbers nikalte hain.
...
│   ├── fusion.py           # Intelligent threat scoring (signal fusion)
│   ├── video.py            # Webcam / video file handler
│   ├── alerts.py           # Cooldown + CSV log + snapshots
│   └── analyzers/
│       ├── loitering.py    # Ek jagah zyada der
│       ├── abandoned.py    # Lawaaris bag
│       ├── crowd.py        # Bheed
│       ├── fall.py         # Girna (box shape se)
│       └── fight.py        # Ladai (paas + tez + der)
│
├── dashboard/
